...
class Test(InputDataset):
    def __init__(self):
        super().__init__("input_dataset", "test", 4, "tbs")
...
def input_dataset_extension_module(tmp_path):
    with open(tmp_path / "test_input_extension.py", "w") as mod_file:
...
def test_input_dataset_extension(monkeypatch, input_dataset_extension_module):
    ...
    monkeypatch.setenv("CHIMP_EXTENSION_MODULES", "test_input_extension")
...
class Test(ReferenceDataset):
    def __init__(self):
        super().__init__("reference_dataset", 4, [])
