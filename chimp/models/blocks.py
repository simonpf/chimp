...
def get_block_factory(name, factory_kwargs=None):
    ...
    if name.lower() in ["simple_conv", "conv2d", "convnet", "unet"]:
        ...
        raise ValueError(
            f"Block type '{name}' is not known. Refer to the 'chimp.blocks' "
...
def get_downsampler_factory(name, factory_kwargs):
    ...
    if name.lower() == "none":
        ...
        raise ValueError(
            ...
            " 'chimp.blocks' module for supported downsamplers."
        )
...
def get_upsampler_factory(upsampling_type, factory_kwargs):
    ...
    if upsampling_type == "bilinear":
        ...
        raise ValueError(
            ...
            " the 'chimp.blocks' module for supported upsampling types."
        )
...
def get_stage_factory(name, factory_kwargs=None):
    ...
    if name.lower() in ["sequential"]:
        ...
        raise ValueError(
            f"Stage architecture '{name}' is not known. Refer to the 'chimp.blocks' "
            " module for supported blocks."
        )
